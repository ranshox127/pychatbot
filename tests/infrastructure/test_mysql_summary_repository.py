...
def clean_course_info(mysql_conn, verify_mysql_conn):
    with mysql_conn.cursor() as cur:
        cur.execute("TRUNCATE TABLE summary_gradding_log")
    mysql_conn.commit()

    with verify_mysql_conn.cursor() as cur:
        cur.execute("TRUNCATE TABLE SummarySubmissions")
    verify_mysql_conn.commit()


def test_get_latest_log_id_returns_correct_log_id(mysql_conn, repo):
    with mysql_conn.cursor() as cur:
        cur.execute("""
...
def test_is_log_under_review_returns_true(verify_mysql_conn, repo):
    with verify_mysql_conn.cursor() as cur:
        cur.execute("""
...
def test_get_score_result_returns_100(mysql_conn, repo):
    with mysql_conn.cursor() as cur:
        cur.execute("""
...
def test_get_score_result_returns_80(mysql_conn, repo):
    with mysql_conn.cursor() as cur:
        cur.execute("""
            INSERT INTO summary_gradding_log (student_ID, context_title, contents_name, result, penalty, operation_time)
...
def test_get_score_result_returns_0_when_no_match(mysql_conn, repo):
    with mysql_conn.cursor() as cur:
        cur.execute("""
            INSERT INTO summary_gradding_log (student_ID, context_title, contents_name, result, penalty, operation_time)
